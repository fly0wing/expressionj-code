 ...
 * @version 0.9.2
 */
public class ExprAddAssignment extends AbstractAssignment {
   
   @Override
   public Object clone() throws CloneNotSupportedException {
      ExprAddAssignment assign = new ExprAddAssignment();
      assign.expr = expr;
   ...
   public int evalAsInt() {
      ...
      resultVariable.setValueAsInt(eval + resultVariable.getValueAsInt());
      return resultVariable.evalAsInt();
   }

   @Override
   public float evalAsFloat() {
      float eval = expr.evalAsFloat();
      resultVariable.setValueAsFloat(eval + resultVariable.getValueAsFloat());
      return resultVariable.evalAsFloat();
   }
   ...
   public Object eval() {
      ...
      if (eval instanceof Integer) {
         ...
         if (value instanceof Integer) {
            evalI = evalI + ((Integer)value).intValue();
            resultVariable.setValue(evalI);
            result = evalI;
         } else if (value instanceof Number) {
            float evalf = evalI + ((Float)value).floatValue();
         ...
         if (value instanceof Number) {
            evalf = evalf + ((Float)value).floatValue();
            resultVariable.setValue(evalf);
            result = evalf;                      
         }
      }
